...
tag_configs = {
    ...
    "string_double": {"foreground": "lightblue"},
    "string_triple": {"foreground": "lightblue"},
    "comma": {"foreground": "lightblue"},
    "variable": {"foreground": _from_rgb((86, 0, 207))},
    "before_method": {"foreground": _from_rgb((192, 0, 240))},
    "after_method": {"foreground": _from_rgb((212, 41, 255))},
}
...
def on_tab_press(event):
    ...
    current_line = editor_text.get(f"{cursor_pos} linestart", f"{cursor_pos} lineend")
...
keywords = ["import ", "from ", "def ", "class ", "if ", "else ", "for ", "while ", "print(", "try ", "except ", "(", ")", "with", "open"]
...
def apply_syntax_highlighting():
    ...
    for start, end, tag_type in zip(
        ...
        ["string_double" if x.group(1) == '"' else "string_triple" for x in string_regex.finditer(current_text)]
    ):
        editor_text.tag_add(tag_type, start, end)

    for match in comma_regex.finditer(current_text):
        editor_text.tag_add("comma", f"1.0+{match.start()}c", f"1.0+{match.end()}c")

    for match in variable_regex.finditer(current_text):
        editor_text.tag_add("variable", f"1.0+{match.start(1)}c", f"1.0+{match.end(1)}c")

    for match in before_method_regex.finditer(current_text):
        editor_text.tag_add("before_method", f"1.0+{match.start(1)}c", f"1.0+{match.end(1)}c")

    for match in after_method_regex.finditer(current_text):
        editor_text.tag_add("after_method", f"1.0+{match.start(1)}c", f"1.0+{match.end(1)}c")
...
editor_text = CTkTextbox(master=editor_frame, fg_color="black", corner_radius=0, font=CTkFont(family="Cascadia Code", size=15))
